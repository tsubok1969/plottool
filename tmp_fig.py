def makefig(jobid, mpi=True, pui=True, endian=True, npui=False, ymin=0, ymax=800, t1=200, t2=600, t3=1400, vmin=0, vmax=1.):
    ...
    c1 = ax1.pcolormesh(X, Y, df1.np, cmap='jet', vmin=vmin, vmax=vmax)
    c2 = ax2.pcolormesh(X, Y, df2.np, cmap='jet', vmin=vmin, vmax=vmax)
    c3 = ax3.pcolormesh(X, Y, df3.np, cmap='jet', vmin=vmin, vmax=vmax)
    ...
    cbar = fig.colorbar(c3, cax = cax, orientation='vertical')
    cbar.ax.ticklabel_format(style='sci', scilimits=(-6,0))
